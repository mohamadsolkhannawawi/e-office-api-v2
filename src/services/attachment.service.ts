...
export class AttachmentService {
	...
	static async uploadAttachment(
		...
		if (file.size > MAX_FILE_SIZE) {
			throw new Error(
				`File ${file.name} terlalu besar. Maksimal 5MB, diterima ${(file.size / 1024 / 1024).toFixed(2)}MB`,
			);
		}

		try {
			// Build MinIO path
			const minioPath = AttachmentService.buildMinioPath(
				userId,
				letterInstanceId,
				category,
				file.name,
			...
			const uploadResult = await new MinioService().uploadFile(
				file,
				`surat-rekomendasi-beasiswa/${userId}/${letterInstanceId}/${category}/`,
			...
			const attachment = await db.attachment.create({
				data: {
					domain: minioPath, // Path di MinIO
	...
	static async getDownloadUrl(
		...
		try {
			...
			const url = await new MinioService().getPresignedUrl(
	...
	static async deleteAttachment(attachmentId: string): Promise<void> {
		try {
			...
			// Delete dari MinIO
			await new MinioService().deleteFile(
				"surat-rekomendasi-beasiswa",
				attachment.filename,
			);
