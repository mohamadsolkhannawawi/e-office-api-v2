...
export default new Elysia()
  ...
  .get(
    ...
    async ({ user, set }) => {
      ...
      if (isMahasiswa && fullUser.mahasiswa) {
        ...
        if (!fullUser.mahasiswa.tahunMasuk) missingFields.push("tahunMasuk");
        // NIM placeholder = prefix email (bukan 14 digit angka)
        if (!/^\d{14}$/.test(fullUser.mahasiswa.nim)) missingFields.push("nim");
        if (!fullUser.mahasiswa.tempatLahir) missingFields.push("tempatLahir");
      ...
      try {
        ...
        if (mahasiswa) {
          ...
          if (body.tanggalLahir)
            mhsData.tanggalLahir = new Date(body.tanggalLahir);
          ...
          if (pegawai) {
            ...
            if (body.programStudiId)
              pgwData.programStudiId = body.programStudiId;
  ...
  .get(
    ...
    async ({ user, set }) => {
      ...
      try {
        const { stat, stream } = await MinioService.getFileStream(objectPath);
