...
/**
 * [ROUTE] User Master Routes
 *
 * Provides endpoints for user management, activity lookup,
 * role-linked data handling (mahasiswa/pegawai), password reset,
 * and status toggling.
 */
export default new Elysia()
  ...
  .get(
    ...
    async ({ params: { id } }) => {
      const activities = await Prisma.letterHistory.findMany({
        where: {
          OR: [{ actorId: id }, { letterInstance: { createdById: id } }],
        },
        include: {
          actor: true,
          letterInstance: {
  ...
  .post(
    ...
    async ({ body }) => {
      ...
      const password =
        initialPassword || `temp${Math.random().toString(36).slice(2, 10)}`;
      ...
      if (assignedRoles.includes("MAHASISWA") && mahasiswaData) {
        console.log(`>>> Creating MAHASISWA data for ${email}:`, mahasiswaData);
        ...
        if (mahasiswaData.nim) {
          ...
          if (existingMahasiswa) {
            throw new Error(`NIM ${mahasiswaData.nim} is already registered`);
          }
        }
        ...
        console.log(`>>> MAHASISWA data created successfully for ${email}`);
      }

      if (pegawaiData && !assignedRoles.includes("MAHASISWA")) {
        console.log(`>>> Creating PEGAWAI data for ${email}:`, pegawaiData);
        ...
        if (pegawaiData.nip) {
          ...
          if (existingPegawai) {
            throw new Error(`NIP ${pegawaiData.nip} is already registered`);
          }
        }

        if (!pegawaiData.departemenId || !pegawaiData.programStudiId) {
          throw new Error(
            "departemenId and programStudiId are required for pegawai",
          );
        }
        ...
        console.log(`>>> PEGAWAI data created successfully for ${email}`);
      }
