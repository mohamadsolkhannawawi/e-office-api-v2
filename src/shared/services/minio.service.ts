...
export abstract class MinioService {
  ...
  public static async getFileStream(location: string) {
    // Ambil statistik file untuk metadata (content type, ukuran, dan lain-lain).
  ...
  /**
   * Ekstrak object key dari presigned URL atau kembalikan apa adanya jika sudah berupa object path.
   * Mendukung URL MinIO lokal maupun URL yang kompatibel dengan S3.
   */
  public static extractObjectKey(urlOrPath: string): string {
    ...
    try {
      ...
      // Cari nama bucket pada path lalu ambil seluruh segmen setelahnya.
      const bucketIndex = pathParts.indexOf(MinioService.bucketName);
      if (bucketIndex !== -1) {
        return pathParts.slice(bucketIndex + 1).join("/");
      }
      // Fallback: anggap segmen pertama adalah bucket, sisanya object key.
  ...
  /**
   * Membuat presigned URL baru dari presigned URL lama atau object path.
   * Ini menyelesaikan masalah URL kedaluwarsa dengan selalu menghasilkan URL yang baru.
   */
